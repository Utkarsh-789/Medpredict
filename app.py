...
def liver_predict():
    try:
        ...
        # ----------------------
        # 🔹 RULE-BASED CHECK (MEDICAL OVERRIDE)
        # ----------------------
        if (
        ...
        # ⚠️ INVERT PROBABILITY (FINAL FIX)
        disease_prob = float(probs[0])   # instead of probs[1]
...
# LOAD MODELS
# =========================
parkinsons_model = joblib.load("models/parkinson_model.pkl")     # calibrated
...
def dashboard():
    ...
    # 🔥 Convert to dictionary (VERY IMPORTANT for clean Jinja use)
